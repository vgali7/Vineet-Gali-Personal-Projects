...
def h(p1, p2):      #swithed from manhattan
    x1, y1 = p1     # distance to euclidean distance
    x2, y2 = p2
    x = ((x1 - x2) ** 2)
    y = ((y1 - y2) ** 2)
    return (x + y) ** 0.5
...
##        for spot in row:
##            if spot.is_closed():
##                spot.reset()  # is not path its being reset  ( for visuals )
##            if spot.is_white():
##                spot.make_closed()
    while current in came_from:
        current = came_from[current]
        if not current.is_start() and not current.is_end():
            current.make_path()  # is not being reser is making path ( for visuals )
...
def a_algorithm(draw, grid, start, end):
    ...
    while not open_set.empty():
        ...
        for neighbor in current.neighbors:
            temp_g_score = g_score[current] + 0.5
